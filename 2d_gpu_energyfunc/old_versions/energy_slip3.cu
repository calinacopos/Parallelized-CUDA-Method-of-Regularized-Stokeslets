...
void velocity(int Npts, int Ntris, vertex Nodes[], vector f[], vector v[]) {
  ...
  double *vx = (double*) malloc(mem_size*add*sizeof(double));
  double *vy = (double*) malloc(mem_size*add*sizeof(double));
  ...
  cudaMalloc((void**) &Vxd, mem_size*add*sizeof(double));
  cudaMalloc((void**) &Vyd, mem_size*add*sizeof(double));

  // Initialize device memory to zero
  cudaMemset(Vxd, 0x0, mem_size*add*sizeof(double));
  cudaMemset(Vyd, 0x0, mem_size*add*sizeof(double));
  ...
  SlipKernel<<< grid, threads >>>(xd, yd, Fxd, Fyd, Vxd, Vyd, visc, e, esq);
  ...
  cudaMemcpy(vx, Vxd, mem_size*add*sizeof(double), cudaMemcpyDeviceToHost);
  cudaMemcpy(vy, Vyd, mem_size*add*sizeof(double), cudaMemcpyDeviceToHost);

  //for(int i=0; i<mem_size*16; i++) {
  //	printf("i: %d, (vx, vy) = (%.16f,%.16f)\n", i, vx[i], vy[i]);
  //}

  // Set velocity
  int numbersideways = Npts/256;
  for(int j=0; j<Npts; j++) {
	int next_down = floor(j/256);
	int next_chunk = floor((j%256)/16)*16;
	for(int l=0; l<(Npts/256); l++) {
	   for(int k=0; k<16; k++) {
	      v[j].x += vx[(j%256) + next_down*numbersideways*256*16 + l*256*16 + next_chunk*15 + k*16];
	      v[j].y += vy[(j%256) + next_down*numbersideways*256*16 + l*256*16 + next_chunk*15 + k*16];
	   }
	}
  ...
  /*for(int j=0; j<256; j++) {
  	...
  	for(int k=0; k<16; k++) {
  	   v[j].x += vx[j+next_chunk*15+k*16];
  	   v[j].y += vy[j+next_chunk*15+k*16];
  	}
  	printf("(vx, vy) = (%.16f, %.16f)\n", v[j].x, v[j].y);
  }*/
