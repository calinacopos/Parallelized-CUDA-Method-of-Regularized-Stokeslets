 ...
 * - works only up to 256 mesh points
 ...
 * - check against cpu & perfect agreement statically & dynamically (2/17/2013)
 */
...
void velocity(int Npts, int Ntris, vertex Nodes[], vector f[], vector v[]) {
  ...
  double *vx = (double*) malloc(mem_size*sizeof(double));
  double *vy = (double*) malloc(mem_size*sizeof(double));
...
void startCurve() {
  ...
  ifstream f2;
  ifstream f3;
  f1.open("data/UnitCirclePointsN256.txt");
  f2.open("data/UnitCircleTrianglesN256.txt");
  ...
  f1.open("data/UnitCirclePointsN256.txt");
  f2.open("data/UnitCircleTrianglesN256.txt");
  f3.open("data/UnitCircleBoundaryN256.txt");
