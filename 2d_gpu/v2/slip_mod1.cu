...
void velocity(double x[N][N], double y[N][N], double vx[], double vy[], int size) {
  ...
  cudaMalloc((void**) &tmpx, (mem_size/16)*(mem_size/16)*sizeof(double));
  cudaMalloc((void**) &tmpy, (mem_size/16)*(mem_size/16)*sizeof(double));
  ...
  SlipKernel<<< grid, threads >>>(xd, yd, Fxd, Fyd, Vxd, Vyd, tmpx, tmpy, N, visc, e, PI);
