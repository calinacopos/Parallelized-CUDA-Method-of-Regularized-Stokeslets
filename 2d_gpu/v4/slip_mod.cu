...
// time 
const float TMAX = 0.1f;
const float tstep = 0.0001f;
...
vector eforce(int i, int j, double x[N*N], double y[N*N]) {
  ...
  if (dlk_00 == 0.0f) { f_00.x = 0.0f; f_00.y = 0.0f; }
	...
	f_00.x = -1.0f*ks*(dlk_00-rd)*((x[i_1+N*j_1] - x[i_0+N*j_0])/dlk_00); 
  	f_00.y = -1.0f*ks*(dlk_00-rd)*((y[i_1+N*j_1] - y[i_0+N*j_0])/dlk_00);
  }

  if (dlk_10 == 0.0f) { f_10.x = 0.0f; f_10.y = 0.0f; }
  else {
	f_10.x = -1.0f*kl*(dlk_10-rl)*((x[i_1+N*j_1] - x[i_1+N*j_0])/dlk_10);
	f_10.y = 1.0f*kl*(dlk_10-rl)*((-y[i_1+N*j_1] + y[i_1+N*j_0])/dlk_10);
  }

  if (dlk_20 == 0.0f) { f_20.x = 0.0f; f_20.y = 0.0f; }
  else {
	f_20.x = -1.0f*ks*(dlk_20-rd)*((x[i_1+N*j_1] - x[i_2+N*j_0])/dlk_20);
        f_20.y = -1.0f*ks*(dlk_20-rd)*((y[i_1+N*j_1] - y[i_2+N*j_0])/dlk_20);
  	
  }
  if (dlk_01 == 0.0f) { f_01.x = 0.0f; f_01.y = 0.0f; }
  else {
	f_01.x = -1.0f*kl*(dlk_01-rl)*((x[i_1+N*j_1] - x[i_0+N*j_1])/dlk_01);
        f_01.y = 1.0f*kl*(dlk_01-rl)*((-y[i_1+N*j_1] + y[i_0+N*j_1])/dlk_01);
  }
  if (dlk_21 == 0.0f) { f_21.x = 0.0f; f_21.y = 0.0f; }
  else {
	f_21.x = -1.0f*kl*(dlk_21-rl)*((x[i_1+N*j_1] - x[i_2+N*j_1])/dlk_21); 
	f_21.y = 1.0f*kl*(dlk_21-rl)*((-y[i_1+N*j_1] + y[i_2+N*j_1])/dlk_21);
  }
  if (dlk_02 == 0.0f) { f_02.x = 0.0f; f_02.y = 0.0f; }
  else {
	f_02.x = -1.0f*ks*(dlk_02-rd)*((x[i_1+N*j_1] - x[i_0+N*j_2])/dlk_02);
	f_02.y = -1.0f*ks*(dlk_02-rd)*((y[i_1+N*j_1] - y[i_0+N*j_2])/dlk_02);	
  }
  if (dlk_12 == 0.0f) { f_12.x = 0.0f; f_12.y = 0.0f; }
  else {
	f_12.x = -1.0f*kl*(dlk_12-rl)*((x[i_1+N*j_1] - x[i_1+N*j_2])/dlk_12);
        f_12.y = 1.0f*kl*(dlk_12-rl)*((-y[i_1+N*j_1] + y[i_1+N*j_2])/dlk_12);
  }
  if (dlk_22 == 0.0f) { f_22.x = 0.0f; f_22.y = 0.0f; }
  else {
	f_22.x = -1.0f*ks*(dlk_22-rd)*((x[i_1+N*j_1] - x[i_2+N*j_2])/dlk_22);
        f_22.y = -1.0f*ks*(dlk_22-rd)*((y[i_1+N*j_1] - y[i_2+N*j_2])/dlk_22);	
  }
...
void progress(double x[N*N], double y[N*N]) {
  ...
  int size = N*N;
  double velx[size]; double vely[size];
  vector ef; 
  double p;
  double ftime = 0.0f;
  ...
  for(int t=0; t<stop; t++) {
    ...
    for(int j=0; j<N; j++) {
	  ...
	  ftime = ftime + partf;
	  x[i+N*j] = x[i+N*j] + tstep*(velx[i+N*j] + ef.x/drag);
          y[i+N*j] = y[i+N*j] + tstep*(vely[i+N*j] + ef.y/drag);
