...
const int N = 2; // # side square partitions 
const double visc = 1.0f;
const double flow = 1.0f;
const double drag = 0.0001f;

// time 
const double TMAX = 0.00001f;/*0.008f;*/
const double tstep = 0.00001f;/*0.0001f;*/
...
void velocity(double x[N][N], double y[N][N], double vx[], double vy[], int size) {
  ...
  for(int j=0; j<N; j++) {  
     for(int i=0; i<N; i++) {
 	   ...
 	   //printf("force @ (%d, %d): (%.10f, %10f)\n", i, j, fxh[i+j*N], fyh[i+j*N]);   
     }
  }
  ...
  double *xd, *yd, *Fxd, *Fyd, *Gxd, *Gyd, *Vxd, *Vyd;
  ...
  TestKernel<<< grid, threads >>> (xd, yd, Fxd, Fyd, Gxd, Gyd, Vxd, Vyd, N, visc, e, PI);
  ...
  double *gxch = (double*) malloc(mem_size*mem_size*sizeof(double));
  double *gych = (double*) malloc(mem_size*mem_size*sizeof(double));
  ...
  cudaMemcpy(gxch, Gxd, mem_size*mem_size*sizeof(double), cudaMemcpyDeviceToHost);
  cudaMemcpy(gych, Gyd, mem_size*mem_size*sizeof(double), cudaMemcpyDeviceToHost);
  ...
  for(int k=0; k<(N*N); k++) {
  	printf("GPU velocity[%d]: (%.10f, %.10f)\n", k, vx[k], vy[k]);
  }
  for(int kk=0; kk<(N*N*N*N); kk++) {
	printf("GPU stokeslet[%d]: (%.10f, %.10f)\n", kk, gxch[kk], gych[kk]);
  }
...
void progress(double x[N][N], double y[N][N]) {
  ...
  for(int t=0; t<stop; t++) {
     ...
     for(int j=0; j<N; j++) {
	   ...
	   //printf("copied velocity: (%.10f, %.10f)\n", velx[i+j*N], vely[i+j*N]);
	   f3 << "tstep: " << tstep << endl;
